...
|---|---|---|---|
| Claude Code | `%USERPROFILE%\.claude\.credentials.json` | `GET https://api.anthropic.com/api/oauth/usage` with `Authorization: Bearer <access_token>` + `anthropic-beta: oauth-2025-04-20`. Primary: top-level `limits[]` array (`kind: session/weekly_all/weekly_scoped`, `percent`, `resets_at`, `scope.model.display_name` for per-model weekly windows, e.g. a "Fable" weekly window). Fallback (older responses, or when `limits` is absent/null/empty): legacy `five_hour`, `seven_day`, `seven_day_sonnet`, `seven_day_opus` fields; `extra_usage`; plan from `subscriptionType` / `rate_limit_tier`. | `%USERPROFILE%\.claude\projects\**\*.jsonl` |
