...
| Grok CLI | `%USERPROFILE%\.grok\auth.json` (keyed by `issuer::client_id`, token field `key`) | `GET https://cli-chat-proxy.grok.com/v1/billing` — unified billing (2026-07): `config.monthlyLimit` + `config.used` + `billingPeriodEnd`; legacy `config.creditUsagePercent` + `currentPeriod.end` still parsed if present — + `GET .../v1/settings` (plan label) | `%USERPROFILE%\.grok\sessions\**\signals.json` (`contextTokensUsed`, `primaryModelId`) |
